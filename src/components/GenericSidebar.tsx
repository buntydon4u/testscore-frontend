...
import { sidebarLinks, SidebarItem, isSidebarParent } from "@/config/sidebarLinks";
import { ChevronLeft, ChevronRight, GraduationCap, LogOut, ChevronDown } from "lucide-react";
...
export const GenericSidebar = () => {
  ...
  // Normalize role - handle both lowercase and uppercase
  ...
  const menuItems = sidebarLinks[userRole as keyof typeof sidebarLinks] || [];
  
  // Get display name for the role
  const roleDisplayName = ROLE_DISPLAY_NAMES[userRole] || 'Portal';

  // If no menu items, show a minimal sidebar
  if (!menuItems || !Array.isArray(menuItems) || menuItems.length === 0) {
    return (
      <div className="bg-slate-800 text-white h-screen fixed left-0 top-0 w-64 flex flex-col z-50">
        <div className="p-4 border-b border-slate-700">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-emerald-500 rounded-lg flex items-center justify-center">
              <GraduationCap className="w-5 h-5" />
            </div>
            <span className="font-bold text-lg">{roleDisplayName}</span>
          </div>
        </div>
